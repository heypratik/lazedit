...
const Header = (props) => {
  ...
  const sheetTriggerRef = useRef(null); // Create a ref for the sheet trigger

  const { emailEditorEl, setLanguage, onJsonUpload } = props;
  const [uploadedJson, setUploadedJson] = useState(null);
  const { setSelectedHtml } = useContext(GlobalContext);
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      if (event.ctrlKey && event.key === 'e') {
        ...
        sheetTriggerRef.current?.click(); // Trigger the sheet to open
      }
    };
  ...
  return (
    <div className="dashboard-header">
      ...
      <div className="dashboard-header-feature">
        ...
        <Button className="bg-black text-white" onClick={() => finishEdit()}>Finish Edit & Continue</Button>
      </div>
    </div>
  );
};
