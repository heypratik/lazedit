  ...
  style="${productBlock.contentStyleConfig.desktop}">
    ${productBlock.list
      .map((productItem) => {
        ...
        return `<div ${productBlock.styleConfig.mobile ? `class="${productBlock.styleConfig.className}"` : ""}
        style="${productBlock.styleConfig.desktop}">
        <img src="${image}" alt="${title}" style="width:100%;max-width:100%;"/>
        <h2>${title}</h2>
        <p>${description}</p>
        <a href="${link} class="${productBlock.styleConfig.className}">${linkText}</a>
      </div>`;
      })
      .join("")}
  </div>`;
