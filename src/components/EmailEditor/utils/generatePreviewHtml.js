...
const createProductsString = (productBlock) => {
  console.log(productBlock)
  const buttonStyle = productBlock.buttonStyles.desktop;
  const styleString = Object.entries(buttonStyle).map(([key, value]) => {
        const kebabCaseKey = key.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
        return `${kebabCaseKey}: ${typeof value == 'number' ? `${value}px` : value};`;
    })
    .join(' ');
  return `<div ${productBlock.contentStyleConfig.mobile ? `class="${productBlock.contentStyleConfig.className}"` : ""} 
  style="${productBlock.contentStyleConfig.desktop}">
    ${productBlock.list
      .map((productItem, index) => {
        const { image, title, description, link, linkText } = productItem;
        return `<table style="margin-top: 10px; margin-bottom: 10px;">
        <tr style="vertical-align: middle;">
                <td style="width:50%; vertical-align: middle;"><img src="${image}" alt="${title}" style="width:100%;max-width:100%;"/></td>
                <td style="width:50%; vertical-align: middle; margin-bottom: 8px; padding: 10px;">
                <div class="product-txtcontent">
                  <h2 style="color:black; margin-top:1rem">${title}</h2>
                  <p style="color:black; margin-bottom:1rem">${description}</p>
                  <a href="${link}" style="${styleString}">${linkText}</a>
                  </div></td>
                </tr>
                </table>`;
      })
...
const blockListToHtml = (blockList) => {
  let content = "";
  blockList.forEach((item) => {
    if (item.key === "column") {
      ...
      style="${item.styleConfig.desktop};width:100%;display:block;">
        ...
        style="width:100%;max-width: auto;margin:0 auto;${item.contentStyleConfig.desktop}">
    ...
    if (item.key === "product") {
      content += createProductsString(item);
    }
  });
...
const generatePreviewHtml = (blockList) => {
  let content = "";
  const { newBlockList, styles } = createStyleTag(blockList);
  content = blockListToHtml(newBlockList);
  return `<html>
  <head>
  <meta charset="UTF-8">
  <title>email</title>
  ...
  *{
  ...
  <body class="email-template mybranz-template">
  <div style="opacity:0;"></div>
  <div style="background-color:#fff;color:#fff; font-family: Arial; max-width: 640px; margin: 0 auto;"> ${content}</div>
  </body>
  </html>`;
};
